...
const LoginForm = (props) => {
    ...
    const [isUsernameInputFirstChange, setIsUsernameInputFirstChange] =
        useState(true);
    const [isPasswordInputFirstChange, setIsPasswordInputFirstChange] =
        useState(true);
    const [
        isRepeatedPasswordInputFirstChange,
        setIsRepeatedPasswordInputFirstChange,
    ] = useState(true);
    ...
    const getIsUsernameInputValid = (inputValue) => {
        if (inputValue === "")
            return setUsernameErrMsg("username cannot be empty");
        if (!/.*[a-zA-Z].*/.test(inputValue))
            return setUsernameErrMsg(
                "username must contain at least one letter"
            );
        if (inputValue.includes(" "))
            return setUsernameErrMsg("username cannot contain spaces");
    ...
    const getIsPasswordInputValid = () => {
        ...
        if (password === "")
            return setPasswordErrMsg("password cannot be empty");
        if (password.includes(" "))
            return setPasswordErrMsg("password cannot contain spaces");
    ...
    const getIsRepeatedPasswordInputValid = (inputValue) => {
        ...
        if (inputValue !== password)
            return setPasswordRepeatErrMsg("passwords do not match");
    ...
    const checkInputsOnSubmit = (e) => {
        e.preventDefault();
        if (isLoading)
            return props.setFormErrMsg(
                `${props.isSignupForm ? "Signup" : "Login"} still processing`
            );
        ...
        const isRepeatedPasswordValid = props.isSignupForm
            ? getIsRepeatedPasswordInputValid(e.target[2].value)
            : true;
        if (!isUsernameValid || !isPasswordValid || !isRepeatedPasswordValid)
            return props.setFormErrMsg("Check inputs errors");
    ...
    return (
        <Form
            ...
            isSubmitButtonEnabled={
                !isUsernameInputFirstChange &&
                !isPasswordInputFirstChange &&
