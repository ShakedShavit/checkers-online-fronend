...
class CheckersLogic {
    ...
    creatingInitialBoardLogic() {
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                if ((j + i) % 2)
                    this.squareObjectsBoard[i][j] = new Square(true, false, i, j, null);
                else this.squareObjectsBoard[i][j] = new Square(false, false, i, j, null);

                if (i < 3 && (j + i) % 2 == 1)
                    this.squareObjectsBoard[i][j].piece = new CheckersPiece(true, false);
                else if (i > 4 && (j + i) % 2 == 1)
                    this.squareObjectsBoard[i][j].piece = new CheckersPiece(false, false);
            }
        }
    }
    ...
    squareOnClick(squareRow, squareColumn) {
        ...
        if (squareObj.isMoveOption) {
            ...
            let movingSquareObj =
                this.squareObjectsBoard[this.lastPieceChosenPosition[0]][
                    this.lastPieceChosenPosition[1]
                ];
            //if the move creates a king
            let kingWasMadeThisMove = false; //to stop multiple jumping after making king
            if (
                this.checkingIfKingWasMadeInMoveAndChangingThePiece(
                    squareRow,
                    movingSquareObj.piece
                )
            )
                kingWasMadeThisMove = true;
            //regular move
            if (Math.abs(movingPieceRowDifference) == 1) {
                squareObj.piece = new CheckersPiece(
                    movingSquareObj.piece.isBlack,
                    movingSquareObj.piece.isKing
                );
            ...
            //eating move
                ...
                let movingPieceColumnDifference =
                    (squareColumn - this.lastPieceChosenPosition[1]) / 2; //for deleting eaten piece
                movingPieceRowDifference /= 2;
                let eatenSquareObj =
                    this.squareObjectsBoard[
                        this.lastPieceChosenPosition[0] + movingPieceRowDifference
                    ][this.lastPieceChosenPosition[1] + movingPieceColumnDifference];
                squareObj.piece = new CheckersPiece(
                    movingSquareObj.piece.isBlack,
                    movingSquareObj.piece.isKing
                );
                ...
                if (
                    kingWasMadeThisMove ||
                    !this.isThereAPossibleEatingMoveThisTurnForASpecificPiece(
                        squareRow,
                        squareColumn
                    )
                ) {
                    ...
                    this.lastPieceThatAtePositionForMultipleEating = "" + squareRow + squareColumn;
            }
    ...
    pieceOnClick(piecePosition) {
        ...
        this.deletingPreviousMovingOptions();
        this.callAddingMovingOptionsToSquaresForRegularMoveIfNotOutOfBorders(
            this.squareObjectsBoard[squareRow][squareColumn],
            false
        ); //adding moving options for regular move for every possible square
        this.callAddMoveOptionFunctionForEveryEatingOption(
            this.squareObjectsBoard[squareRow][squareColumn],
            false
        ); //adding eating options for regular move for every possible square
    ...
    addMovingOptionClassToSquare(
        targetSquarePositionRow,
        targetSquarePositionColumn,
        checkingMovingPossibilitiesForTieCheck
    ) {
        let moveOptionSquareObj =
            this.squareObjectsBoard[targetSquarePositionRow][targetSquarePositionColumn];
    ...
    addMovingOptionClassToSquareForEatingMove(
        moveOptionSquareObj,
        squareEatenObj,
        squareEatingObj
    ) {
        if (this.isEatingPossible(moveOptionSquareObj, squareEatenObj, squareEatingObj)) {
            if (this.lastPieceThatAtePositionForMultipleEating !== "") {
                if (
                    this.lastPieceThatAtePositionForMultipleEating ===
                    "" + squareEatingObj.row + squareEatingObj.column
                )
                    moveOptionSquareObj.isMoveOption = true;
    ...
    getIdOfSquaresForEatingIfNotOutOfBorders(eatingSquareObj) {
        ...
        if (!this.isRedPlayersTurn && eatingPieceObj.isBlack) {
            ...
            if (eatingPieceObj.isKing)
                resultReturn = this.eatingMoveForRedPiece(
                    positionRow,
                    positionColumn,
                    resultReturn
                );
            ...
            if (eatingPieceObj.isKing)
                resultReturn = this.eatingMoveForBlackPiece(
                    positionRow,
                    positionColumn,
                    resultReturn
                );
    ...
    callAddingMovingOptionsToSquaresForRegularMoveIfNotOutOfBorders(
        movingSquareObj,
        checkingMovingPossibilitiesForTieCheck
    ) {
        //this guy
        ...
        if (!this.isRedPlayersTurn && movingPieceObj.isBlack) {
            if (checkingMovingPossibilitiesForTieCheck && !movingPieceObj.isKing)
                return this.regularMovingForBlackPiece(
                    squareRow,
                    squareColumn,
                    checkingMovingPossibilitiesForTieCheck
                );
                ...
                if (
                    this.regularMovingForBlackPiece(
                        squareRow,
                        squareColumn,
                        checkingMovingPossibilitiesForTieCheck
                    )
                    ...
                    return true;
                return this.regularMovingForRedPiece(
                    squareRow,
                    squareColumn,
                    checkingMovingPossibilitiesForTieCheck
                );
            ...
            if (movingPieceObj.isKing)
                this.regularMovingForRedPiece(
                    squareRow,
                    squareColumn,
                    checkingMovingPossibilitiesForTieCheck
                );
    ...
    regularMovingForBlackPiece(squareRow, squareColumn, checkingMovingPossibilitiesForTieCheck) {
        if (squareRow + 1 < 8 && squareColumn + 1 < 8) {
            if (
                checkingMovingPossibilitiesForTieCheck &&
                this.addMovingOptionClassToSquare(
                    squareRow + 1,
                    squareColumn + 1,
                    checkingMovingPossibilitiesForTieCheck
                ) === true
    ...
    iteratingOverAllSquaresAndCheckingEatingPossibility() {
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                if (this.isThereAPossibleEatingMoveThisTurnForASpecificPiece(i, j) === true)
                    return true;
            }
        }
        return false;
    ...
    iteratingOverAllSquaresAndCheckingRegularMovingPossibility() {
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                if (this.isThereAPossibleRegulargMoveThisTurnForASpecificPiece(i, j) === true)
                    return true;
            }
        }
        return false;
    ...
    isThereAPossibleEatingMoveThisTurnForASpecificPiece(checkingSquareRow, checkingSquareColumn) {
        ...
        if (checkingSquarePieceObj !== null) {
            if (
                (!this.isRedPlayersTurn && checkingSquarePieceObj.isBlack) ||
                (this.isRedPlayersTurn && !checkingSquarePieceObj.isBlack)
            ) {
    ...
    isThereAPossibleRegulargMoveThisTurnForASpecificPiece(checkingSquareRow, checkingSquareColumn) {
        ...
        if (checkingPieceObj !== null) {
            if (
                (!this.isRedPlayersTurn && checkingPieceObj.isBlack) ||
                (this.isRedPlayersTurn && !checkingPieceObj.isBlack)
            ) {
                return this.callAddingMovingOptionsToSquaresForRegularMoveIfNotOutOfBorders(
                    checkingSquareObj,
                    true
                );
            }
    ...
    callAddMoveOptionFunctionForEveryEatingOption(
        checkingSquareObj,
        didMoveForSpecificPieceFuncCalled
    ) {
        ...
        if (eatingMoveSquares.length > 0) {
            ...
            if (this.isEatingPossible(moveOptionSquareObj, squareEatenObj, checkingSquareObj)) {
                if (didMoveForSpecificPieceFuncCalled) return true;
                    ...
                    this.addMovingOptionClassToSquareForEatingMove(
                        moveOptionSquareObj,
                        squareEatenObj,
                        checkingSquareObj
                    ); //marking squares function calling
            }
    ...
    checkingIfKingWasMadeInMoveAndChangingThePiece(moveOptionSquarePositionRow, movingPieceObj) {
        if (
            (moveOptionSquarePositionRow === 0 || moveOptionSquarePositionRow === 7) &&
            !movingPieceObj.isKing
        ) {
        ...
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                if (this.squareObjectsBoard[i][j].piece !== null) {
                    if (this.squareObjectsBoard[i][j].piece.isBlack === !this.isRedPlayersTurn)
                        return false;
                }
            }
        }
